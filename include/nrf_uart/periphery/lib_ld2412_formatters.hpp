...
template<>
struct tools::formatter_t<LD2412::LightSensitivity>
{
    template<FormatDestination Dest>
    static std::expected<size_t, FormatError> format_to(Dest &&dst, std::string_view const& fmtStr, LD2412::LightSensitivity const& p)
    {
        const char *pStr = "<unk>";
        switch(p)
        {
            case LD2412::LightSensitivity::Off: pStr = "Off"; break;
            case LD2412::LightSensitivity::DetectWhenBiggerThan: pStr = "BiggerThan"; break;
            case LD2412::LightSensitivity::DetectWhenLessThan: pStr = "LessThan"; break;
        }
        return tools::format_to(std::forward<Dest>(dst), "{}", pStr);
    }
};
